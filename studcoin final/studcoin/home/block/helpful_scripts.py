...
def intial_issue(address):
    account= get_account()
    print(f"Issue Tokens called by: {account}")
    token_farm  = get_contract("ThaparToken")
    print(f"ThaparToken contract called to issue tokens: {token_farm}")
    tx = token_farm.intialIssue(address,{"from": account})


def cgpa_dis(address , amount) :
    account= get_account()
    print(f"Issue Tokens called by: {account}")
    token_farm  = get_contract("ThaparToken")
    print(f"ThaparToken contract called to issue tokens: {token_farm}")
    tx = token_farm.CGPADistribute(address ,amount,{"from": account})


def inter_college_exchange(address1 , address2, amount1, amount2):
    account= get_account()
    print(f"Issue Tokens called by: {account}")
    token_farm  = get_contract("ThaparToken")
    print(f"ThaparToken contract called to issue tokens: {token_farm}")
    tx = token_farm.interCollegeTransfer(address1 ,address2,amount1,amount2,{"from": account})
    tx.wait(1)
def buypass(amt):
    account= get_account()
    print(f"Issue Tokens called by: {account}")
    token_farm  = get_contract("ThaparToken")
    print(f"ThaparToken contract called to issue tokens: {token_farm}")
    tx = token_farm.buyPassThapar(amt, {"from":account})
    tx.wait(1)
